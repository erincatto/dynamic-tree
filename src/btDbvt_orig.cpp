...
//
static inline void insertleaf(btDbvt* pdbvt, btDbvt::Node* root, btDbvt::Node* leaf)
{
	if (!pdbvt->m_root)
	{
		pdbvt->m_root = leaf;
	...
	else
	{
		...
		{
			do {
				if (proximity(root->childs[0]->box, leaf->box) < proximity(root->childs[1]->box, leaf->box))
					root = root->childs[0];
				else
					root = root->childs[1];
			} while (!root->isleaf());
...
//
static btDbvt::Node* topdown(btDbvt* pdbvt, tNodeArray& leafs)
{
